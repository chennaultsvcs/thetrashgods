...
class OAuth2 implements FetchAuthTokenInterface
{
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        if (\class_exists('Sgdg\\Vendor\\Firebase\\JWT\\JWT')) {
            return \Sgdg\Vendor\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
        }
        return \Sgdg\Vendor\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm, $signingKeyId = null)
    {
        if (\class_exists('Sgdg\\Vendor\\Firebase\\JWT\\JWT')) {
            return \Sgdg\Vendor\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
        }
        return \Sgdg\Vendor\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
    }
    /**
