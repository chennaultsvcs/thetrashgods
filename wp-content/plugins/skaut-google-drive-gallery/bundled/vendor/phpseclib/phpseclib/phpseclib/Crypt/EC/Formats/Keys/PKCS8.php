...
abstract class PKCS8 extends Progenitor
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (!$params) {
            throw new \RuntimeException('Sgdg\\Vendor\\Unable to decode the parameters using Maps\\ECParameters');
        }
        $components = [];
    ...
    public static function savePrivateKey(Integer $privateKey, BaseCurve $curve, array $publicKey, $password = '', array $options = [])
    {
        ...
        if ($curve instanceof TwistedEdwardsCurve) {
            return self::wrapPrivateKey("\x04 " . $privateKey->secret, [], null, $password, $curve instanceof Ed25519 ? 'id-Ed25519' : 'id-Ed448', "\x00" . $curve->encodePoint($publicKey));
        }
